...
function HomeworkWidget() {
  const [hw, setHw] = useState<Array<{id:number;subject:string;title:string;dueDate:string}>>([]);
  useEffect(() => { fetch("/api/homework").then(r => r.json()).then(d => { if (Array.isArray(d)) setHw(d.slice(0, 4)); }); }, []);
  ...
  return (
    <div className="liquid-glass-strong rounded-3xl p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1"><h3 className="text-lg font-bold text-charcoal">Today Homework</h3><p className="text-xs text-muted">Latest assignments</p></div>
        <a href="/admin/homework" className="text-xs font-semibold text-royal hover:underline">Manage</a>
      </div>
      <div className="space-y-2">{hw.map(h => (<div key={h.id} className="flex items-center gap-3 bg-white/50 rounded-2xl p-3"><span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-royal/10 text-royal shrink-0">{h.subject}</span><span className="text-sm font-medium text-charcoal truncate">{h.title}</span>{h.dueDate && <span className="text-[10px] text-muted ml-auto shrink-0">{h.dueDate}</span>}</div>))}</div>
    </div>
  );
}
