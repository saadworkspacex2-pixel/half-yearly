...
export default function Leaderboard3DPodium({ top3, onSelectStudent }: Leaderboard3DPodiumProps) {
  ...
  const podiumSlots = [
    { student: second, rankLabel: "2nd", crown: <SilverCrown />, heightClass: "h-36 sm:h-44 md:h-52", color: "from-[#1e293b] to-[#0f172a]", borderColor: "border-slate-600/60", ringColor: "ring-slate-300" },
    { student: first, rankLabel: "1st", crown: <GoldCrown />, heightClass: "h-48 sm:h-56 md:h-64", color: "from-[#1e3a8a] to-[#0f172a]", borderColor: "border-blue-500/70", ringColor: "ring-amber-400" },
    { student: third, rankLabel: "3rd", crown: <BronzeCrown />, heightClass: "h-28 sm:h-36 md:h-44", color: "from-[#271e3b] to-[#0f172a]", borderColor: "border-amber-800/60", ringColor: "ring-amber-600" },
  ];

  return (
    <div className="relative w-full rounded-3xl md:rounded-[2.5rem] bg-[#070b19] p-6 sm:p-8 md:p-12 overflow-hidden shadow-2xl border border-blue-900/40">
      {/* Ambient Radial Rays / Spotlight Backdrop */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {/* Central Cone Beam */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[500px] bg-gradient-to-b from-blue-500/20 via-indigo-500/5 to-transparent blur-3xl rounded-full" />
        {/* Radial Light Rays */}
        <div 
          className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[800px] opacity-15"
          style={{
            background: "radial-gradient(circle, rgba(99,102,241,0.4) 0%, transparent 70%), repeating-conic-gradient(from 0deg, rgba(255,255,255,0.08) 0deg 15deg, transparent 15deg 30deg)"
          }}
        />
      </div>
      ...
      <div className="relative z-10 text-center mb-8 sm:mb-12">
        <span className="inline-block px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold tracking-widest text-amber-400 bg-amber-500/10 border border-amber-500/20 uppercase mb-2">
          ✨ Champions Arena
        </span>
        <h2 className="text-xl sm:text-3xl font-black text-white tracking-tight">Top Performers</h2>
      </div>
      ...
      <div className="relative z-10 flex flex-row items-end justify-center gap-2 sm:gap-4 md:gap-8 max-w-3xl mx-auto pt-6">
        {podiumSlots.map(({ student, rankLabel, crown, heightClass, color, borderColor, ringColor }, idx) => {
          ...
          const pointsVal = student.points ?? (student.totalObtained ? student.totalObtained * 10 : student.gpa ? Math.round(student.gpa * 500) : 2500);
          const levelVal = student.level ?? Math.max(1, Math.floor((student.totalObtained ?? 100) / 25) || 24);
          ...
          return (
            ...
            >
              ...
              <div className="relative mb-2 shrink-0">
                <div className={`w-14 h-14 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full bg-slate-800 ring-4 ${ringColor} shadow-[0_0_25px_rgba(255,255,255,0.15)] flex items-center justify-center overflow-hidden transition-transform duration-300 group-hover:scale-105`}>
                  {student.profilePicture ? (
                    <img src={student.profilePicture} alt={student.name} className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-xl sm:text-3xl font-black text-white">{student.name.charAt(0)}</span>
                  )}
                </div>

                {/* Pink Level Badge */}
                <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-[9px] sm:text-xs font-black px-2 py-0.5 rounded-md shadow-lg border border-pink-300/40 tracking-wider">
                  {levelVal}
                </div>
              </div>
              ...
              <div className="text-center mt-2 mb-1 w-full px-1">
                <p className="text-xs sm:text-base font-extrabold text-white truncate group-hover:text-amber-300 transition-colors">
                  @{student.name.toLowerCase().replace(/\s+/g, "")}
                </p>
                <p className="text-[10px] sm:text-xs text-slate-400 truncate">{student.name}</p>
              </div>

              {/* Score / Points Pill Badge */}
              <div className="mb-3 inline-flex items-center gap-1.5 bg-[#141b36] border border-blue-500/30 rounded-full px-2.5 py-1 shadow-inner">
                <span className="w-4 h-4 rounded-full bg-amber-400 text-[#7c2d12] flex items-center justify-center font-black text-[10px] shadow">
                  🪙
                </span>
              ...
              {/* 3D Pedestal Block */}
              <div className={`w-full ${heightClass} ${color} ${borderColor} border-t-2 border-x-2 rounded-t-2xl sm:rounded-t-3xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] relative flex flex-col items-center justify-center overflow-hidden transition-all duration-300 group-hover:brightness-110`}>
                {/* 3D Embossed Front Text */}
                <span className="text-3xl sm:text-5xl md:text-6xl font-black text-slate-700/40 select-none tracking-tighter drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">
                  {rankLabel}
                </span>
